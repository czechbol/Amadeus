...
class Vote(basecog.Basecog):
            ...
            if emote is not None:
                for emoji in self.bot.emojis:
                    if str(emoji) in line:
                        votes.append({"emote": emoji, "option": line.replace(
                            str(emoji)+" ", ""), "num_votes": 0})
                        break
        while True:
            if date < datetime.now():
                break
            timeout = (date - datetime.now()).seconds
            pending_tasks = [self.bot.wait_for('raw_reaction_add'),
    ...
    @commands.command(rest_is_raw=True, description=text.get("vote", "vote_desc"), help=text.fill("vote", "vote_help", prefix=config.prefix))
            ...
            for v in lis:
                try:
                    ...
                    task = asyncio.create_task(
                        self.loop(vote_msg, edit_msg, date))
