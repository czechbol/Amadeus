...
class VoteRepository(BaseRepository):
    ...
    def add_vote(self, channel_id: int,
        message_id: int, edit_id: int, date: str):
        """Add new vote"""
        session.add(Vote(channel_id=channel_id, message_id=message_id,
            edit_id=edit_id, date=date))
        session.commit()
    ...
    def del_vote(self, channel_id: int = None, message_id: int = None):
        users = session.query(Vote).filter(Vote.message_id == message_id).\
